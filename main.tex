...
\subsection{Hybrid Time Domain Preliminaries }
...
\subsection{Hybrid Time Solutions}
A solution of $\mathcal{H}$ is a map $\phi:E \mapsto \mathbb{R^n}$ where
$\phi$ is a hybrid arc and satisfies hybrid basic conditions of
\cite{hybrid_2012}.

\subsection{STL Definitions} 

Recall the state of a hybrid system is $x\in \mathbb{R}^n$.

\begin{definition}{(Atomic Proposition)}

An atomic proposition $p^{\mu}$ is a function that maps the domain of
the system to a boolean value. 
\begin{equation}
    p^{\mu}\;:\; \mathcal{Z}\times\mathcal{U}\mapsto \{0,1\}
\end{equation}
\end{definition}

$\mu:\mathcal{Z}\times\mathcal{U}\mapsto\mathbb{R}$ represents a robustness measure of the proposition such that:
Where,
\begin{center}
    $\mu > 0 \implies p^{\mu} = 1$\\
